...
One of the main assumptions in where March Madness gets it's name is from the amount close games in the tournament.
...
I chose not to focus on the regular season OT data collected prior to the 2000 season.
As the plot indicates there is a steep uptick in the percent of games requiring OT.
I attribute this to data collection/preperation error rather than an actual change in how often games go to OT.
The same can be said about the most recent 2019 seasons data.
...
An indication that games are closer in March Madness would be if the density curves below for March Madness were bunched up near the y-axis and the regular season curves were to the right.
That would indicate the difference in scores (average, final, and max) were all near zero (tied game) during March Madness.
...
A Comeback in sports can be one of the most exhilirating phenomenons to watch.
If you are a fan of the team coming back you watch with excitement and hope as your team closes the gap.
If you are a fan of the opposite team you can only watch in horror as your team blows what seemed like an insurmountable lead only minutes ago.
...
March Madness is made for comebacks.
Even when all seems lost, teams continue fighting and scraping to get back in the game one point at a time.
The plots below show the percentage of games with comebacks by teams down at least 15 points.
The plot on the left includes comebacks that started at any time during the game.
The plot on the right only includes comebacks that started in the second half.
In 2015 & 16 March Madness saw higher rates in these comebacks.
Since then the tournament has not produced comebacks as frequently.
The regular season has held steady in both types of comebacks with a spike in the 2019 season.
...
The next plot shows the percentage of games where the winner came back from 2 points down in the final three seconds.
Often times these winning buckets are referred to as "buzzer beaters" since they are taken just in time before the final game buzzer sounds.
The plot below shows that the regular season typically sees 3-4% of games come down to a "buzzer beater" while March Madness varies from year to year.
In 2018 6% of the games in March Madness ended with a "buzzer beater" while the previous (2017) and following (2019) years saw a low percentage (<2%) of games ending with a "buzzer beater".
